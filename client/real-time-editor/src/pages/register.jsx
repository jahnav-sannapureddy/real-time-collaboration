  ...
  return (
    <div className="flex w-screen h-[calc(100vh-4rem)] justify-center items-center">
        <div className="flex flex-col m-8  rounded-lg w-96 overflow-hidden border-indigo-800 b border-2 justify-center">
          <span className="text-center py-3 text-lg bg-indigo-800 text-white">SIGN UP</span>
            {showMessage && <span className={`text-white bg-red-400 text-lg mx-8 mt-4 p-2`}>{ showMessage && `${message}`}</span>}
            <form className="flex flex-col px-8 my-4">
              ...
              <input id="email"
                className="mb-4 bg-gray-50 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" type="email" placeholder="example@mail.com"
                value={email}
                onChange={(e) => {
              ...
              />
              <label for="cnfrmpassword" className="block mb-1">Confirm Password</label>
              ...
              <Link className="m-2 self-center" to="/login">Already have an account? <span className="text-indigo-500">Login</span></Link>
            </form>
        </div>
    </div>
  );
